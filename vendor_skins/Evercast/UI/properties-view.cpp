				    ...
				    QFormLayout *layout)
{
	...
	if (!label && type != OBS_PROPERTY_BOOL && type != OBS_PROPERTY_BUTTON)
...
void WidgetInfo::EditListAddText()
{
	...
	EditableItemDialog dialog(((QWidget *)object)->window(), QString(),
				  false);
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		QString path = QFileDialog::getOpenFileName(
			App()->GetMainWindow(), QTStr("Browse"), item->text(),
			QT_UTF8(filter));
		if (path.isEmpty())
			return;
