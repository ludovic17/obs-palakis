...
bool EvercastWebsocketClientImpl::connect(const std::string &raw_url,
					  ...
					  WebsocketClient::Listener *listener)
{
	...
	try {
		...
		client.set_close_handler(std::bind(
			&EvercastWebsocketClientImpl::handleDisconnect, this,
			_1, listener));

		// --- Failure handler
		client.set_fail_handler(
			std::bind(&EvercastWebsocketClientImpl::handleFail,
				  this, _1, listener));
...
// Core or videoRoom?
void EvercastWebsocketClientImpl::sendLoginMessage(std::string username,
						   std::string token,
						   std::string room)
{
	// Login command
...
bool EvercastWebsocketClientImpl::sendTrickleMessage(
	const std::string &mid, int index, const std::string &candidate,
	bool last)
{
	json trickle;
	if (!last) {
...
bool EvercastWebsocketClientImpl::sendOpenMessage(const std::string &sdp,
